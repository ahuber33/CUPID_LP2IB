...
void OpticalSimulationSteppingAction::CountReemission(
    const G4Step *aStep, OpticalSimulationEventAction *evtac) {
    if (VerbosityLevel>2){
...
void OpticalSimulationSteppingAction::SetPhotonDetectedInformation(
    const G4Step *aStep, OpticalSimulationEventAction *evtac) {
    if (VerbosityLevel>2){
...
void OpticalSimulationSteppingAction::UserSteppingAction(const G4Step *aStep) {
    ...
    static const std::map<std::string,
        ...
        ScMap = {
            {"LMO", &OpticalSimulationEventAction::GetLMO},
        };
